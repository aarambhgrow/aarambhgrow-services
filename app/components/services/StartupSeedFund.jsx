...
import { CheckCircle2, ShieldCheck, Target, FileText, Award, ChevronDown, WalletCards, IndianRupee, Rocket, Lightbulb } from "lucide-react";

import { startupSeedFundData } from "../../data/startup-seed-fund";

export default function StartupSeedFund() {
  const services = Array.isArray(startupSeedFundData?.services) ? startupSeedFundData.services : [];

  const categories = Array.isArray(startupSeedFundData?.categories) ? startupSeedFundData.categories : [];

  const standaloneServiceIds = ["startup-seed-fund"];

  const standaloneServiceNames = ["Startup Seed Fund", "Startup Seed Funding", "Seed Fund", "Seed Funding"];

  const allowedCategoryTitles = ["Startup Funding", "Startup Finance", "Investment Support", "Government Funding", "Government Finance"];
  ...
  if (!startupSeedFundData || services.length === 0 || !currentService) {
    return (
      <section className="w-full bg-[#F8FAFC] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-md border border-red-200 bg-red-50 p-6">
            <h2 className="text-lg font-bold text-red-700">Startup Seed Fund data not found</h2>

            <p className="mt-2 text-sm text-red-600">
              Please check your <strong>data/startup-seed-fund.js</strong> file and make sure the services array contains the Startup Seed
              Fund service.
            </p>
          </div>
        </div>
  ...
  const handleServiceClick = (serviceIndex, categoryTitle = "") => {
    ...
    if (typeof window !== "undefined" && window.innerWidth < 1024) {
      setTimeout(() => {
        document.getElementById("startup-seed-fund-content")?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }, 100);
  ...
  return (
    <section id="startup-seed-fund" className=" relative w-full bg-[#F8FAFC] text-[#03254C] font-sans py-10 sm:py-12 lg:py-16 ">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* =====================================================
            INTRO
        ===================================================== */}
        ...
        >
          ...
          >
            {startupSeedFundData.hero?.title || "Startup Seed Fund — Early-Stage Startup Funding"}
          </motion.h1>
          ...
          >
            {startupSeedFundData.hero?.description}
          </motion.p>
        </motion.div>
        ...
        >
          <motion.div variants={fadeUp}>
            <h2 className=" text-[20px] sm:text-[24px] lg:text-[28px] font-extrabold tracking-[-0.02em] leading-[1.2] text-[#03254C] mb-3 ">
              Startup Seed Fund for Early-Stage Businesses
            </h2>

            <p className=" text-[12px] sm:text-[13px] lg:text-[14px] leading-[1.75] text-[#475569] ">
              Startup seed funding can provide important financial support during the early stages of a business when founders are
              developing products, validating ideas, building prototypes, testing markets and preparing for commercialisation. AarambhGrow
              provides structured startup funding support to help eligible businesses identify suitable funding opportunities, organize
              required information and prepare professionally for the funding application process.
            </p>
          </motion.div>
          ...
          <motion.div variants={fadeUp}>
            ...
            <p className=" text-[12px] sm:text-[13px] lg:text-[14px] leading-[1.75] text-[#475569] ">
              Early-stage startups may explore different funding routes depending on their business stage, innovation, product development
              status, market opportunity and capital requirement. Suitable opportunities can include startup seed funds, government-backed
              programmes, incubators, accelerators and other early-stage investment channels. Funding remains subject to the eligibility
              criteria and evaluation process of the respective programme or funding provider.
            </p>
          </motion.div>

          <motion.div variants={fadeUp}>
            <h2 className=" text-[20px] sm:text-[24px] lg:text-[28px] font-extrabold tracking-[-0.02em] leading-[1.2] text-[#03254C] mb-3 ">
              Prototype, Proof of Concept & Product Development Funding
            </h2>

            <p className=" text-[12px] sm:text-[13px] lg:text-[14px] leading-[1.75] text-[#475569] ">
              Seed-stage capital is often used during critical startup development activities such as proof of concept creation, prototype
              development, product trials, market validation, initial market entry and commercialisation. AarambhGrow helps startups
              structure their project requirements, funding objectives and supporting business information for relevant seed funding
              opportunities.
            </p>
          </motion.div>
          ...
          <motion.div variants={fadeUp}>
            <h2 className=" text-[20px] sm:text-[24px] lg:text-[28px] font-extrabold tracking-[-0.02em] leading-[1.2] text-[#03254C] mb-3 ">
              Startup Funding Application & Documentation Support
            </h2>

            <p className=" text-[12px] sm:text-[13px] lg:text-[14px] leading-[1.75] text-[#475569] ">
              A properly prepared startup funding application should clearly communicate the business model, product or technology, target
              market, customer opportunity, funding requirement, use of funds and expected growth. Our support helps organize business
              documents, financial information, project details and funding-related information to improve application readiness.
            </p>
          </motion.div>
        </motion.div>
        ...
        <div className=" grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 items-start ">
          ...
          <aside className=" lg:col-span-1 bg-white border border-[#E2E8F0] rounded-md p-3 sm:p-4 shadow-[0_2px_10px_rgba(15,23,42,0.03)] lg:sticky lg:top-6 ">
            <div className=" flex items-start gap-3 p-3 rounded-md bg-[#F8FAFC] border border-[#E2E8F0] mb-3 ">
              <div className=" w-8 h-8 rounded-md bg-[#157327]/10 flex items-center justify-center shrink-0 ">
                <Rocket className=" w-4 h-4 text-[#157327] " />
              </div>

              <div>
                <h3 className=" text-[11px] sm:text-xs font-bold leading-[1.35] text-[#03254C] ">Startup Funding Solutions</h3>
              </div>
            </div>
          ...
          <div id="startup-seed-fund-content" className=" lg:col-span-3 scroll-mt-24 ">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {Array.isArray(currentService.fundingAreas) && currentService.fundingAreas.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    <div className=" flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0] ">
                      <div className=" w-8 h-8 rounded-md bg-[#F26522]/10 flex items-center justify-center shrink-0 ">
                        <IndianRupee className=" w-4 h-4 text-[#F26522] " />
                      </div>

                      <h3 className=" text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C] ">
                        Startup Funding Areas
                      </h3>
                    </div>

                    <div className=" flex flex-wrap gap-2 ">
                      {currentService.fundingAreas.map((item, index) => (
                        <span
                          key={`${item}-${index}`}
                ...
                {Array.isArray(currentService.fundingTypes) && currentService.fundingTypes.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    <div className=" flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0] ">
                      <div className=" w-8 h-8 rounded-md bg-[#157327]/10 flex items-center justify-center shrink-0 ">
                        <Lightbulb className=" w-4 h-4 text-[#157327] " />
                      </div>

                      <h3 className=" text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C] ">
                        Suitable Seed Funding Routes
                      </h3>
                    </div>

                    <div className=" grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 ">
                      {currentService.fundingTypes.map((item, index) => (
                        <div
                          key={`${item}-${index}`}
                ...
                {Array.isArray(currentService.process) && currentService.process.length > 0 && (
                  ...
                  >
                    <div className=" flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0] ">
                      ...
                      <h3 className=" text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C] ">
                        Our Startup Seed Funding Process
                      </h3>
                    </div>
                ...
                >
                  <div className=" flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0] ">
                    ...
                    <h3 className=" text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C] ">
                      Why Startups Choose Seed Funding Support
                    </h3>
                  </div>
