...
import { CheckCircle2, ShieldCheck, Target, FileText, Award, ChevronDown, WalletCards, IndianRupee, Factory, Utensils } from "lucide-react";
import { pmfmeData } from "../../data/pmfme";

export default function PMFME() {
  const services = Array.isArray(pmfmeData?.services) ? pmfmeData.services : [],
    categories = Array.isArray(pmfmeData?.categories) ? pmfmeData.categories : [],
    standaloneServiceIds = ["pmfme"],
    standaloneServiceNames = [
      "PMFME",
      "PMFME Scheme",
      "PMFME Loan",
    ...
    allowedCategoryTitles = ["Food Processing Finance", "Government Finance", "Business Finance", "Enterprise Finance"];
  const isStandaloneService = (service) => {
    if (!service) return false;
    const serviceId = String(service.id || "")
        .toLowerCase()
        .trim(),
      serviceName = String(service.name || service.title || service.shortTitle || "")
        .toLowerCase()
        .trim();
  ...
  const [activeTab, setActiveTab] = useState(0),
    [openCategory, setOpenCategory] = useState(categoryNavigation?.[0]?.title || "");
  const currentService = services[activeTab] || services[0] || null;
  if (!pmfmeData || services.length === 0 || !currentService)
    return (
      <section className="w-full bg-[#F8FAFC] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-md border border-red-200 bg-red-50 p-6">
            <h2 className="text-lg font-bold text-red-700">PMFME data not found</h2>
            <p className="mt-2 text-sm text-red-600">
              Please check your <strong>data/pmfme.js</strong> file and make sure the services array contains the PMFME service.
            </p>
          </div>
        </div>
      </section>
  ...
  const handleServiceClick = (serviceIndex, categoryTitle = "") => {
    ...
    if (typeof window !== "undefined" && window.innerWidth < 1024)
      setTimeout(() => document.getElementById("pmfme-content")?.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  };
  const containerVariants = { hidden: {}, visible: { transition: { staggerChildren: 0.08, delayChildren: 0.05 } } },
    fadeUp = { hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0, transition: { duration: 0.45, ease: [0.25, 0.1, 0.25, 1] } } },
    cardVariants = {
  ...
  return (
    <section id="pmfme" className="relative w-full bg-[#F8FAFC] text-[#03254C] font-sans py-10 sm:py-12 lg:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
        ...
        >
          ...
          >
            {pmfmeData.hero?.title || "PMFME Scheme for Micro Food Processing Businesses"}
          </motion.h1>
        ...
        >
          {[
            [
              "PMFME Scheme for Micro Food Processing Businesses",
            ...
            [
              "PMFME Loan & Subsidy Support",
              "Eligible micro food processing enterprises may access financial support under applicable PMFME guidelines, including credit-linked assistance and other permitted support mechanisms. The availability, amount and conditions of support depend on the current scheme guidelines, applicant eligibility and implementing authorities.",
            ],
            [
              "PMFME Eligibility & Application Preparation",
              "PMFME eligibility can depend on the enterprise profile, food processing activity, project requirements, location, applicant category and prevailing scheme conditions. We help applicants understand the applicable requirements and organize the information needed for a stronger, more complete application.",
            ],
            [
              "PMFME Project Report & Food Processing Business Support",
              "A properly prepared project report can clearly present the proposed food processing activity, machinery or equipment, project cost, working capital, production capacity, market opportunity, revenue assumptions and other relevant business details. AarambhGrow helps structure these details for PMFME application readiness.",
            ],
          ].map(([title, text]) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 items-start">
          <aside className="lg:col-span-1 bg-white border border-[#E2E8F0] rounded-md p-3 sm:p-4 shadow-[0_2px_10px_rgba(15,23,42,0.03)] lg:sticky lg:top-6">
            <div className="flex items-start gap-3 p-3 rounded-md bg-[#F8FAFC] border border-[#E2E8F0] mb-3">
              ...
              <div>
                <h3 className="text-[11px] sm:text-xs font-bold leading-[1.35] text-[#03254C]">Food Processing Finance</h3>
              </div>
            </div>
                        ...
                        >
                          {category.services.map((service) => {
                            const serviceIndex = services.findIndex((item) => item.id === service.id),
                              isActive = activeTab === serviceIndex;
                            return (
                              <button
                                type="button"
                                key={service.id}
          ...
          <div id="pmfme-content" className="lg:col-span-3 scroll-mt-24">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {Array.isArray(currentService.foodProcessingActivities) && currentService.foodProcessingActivities.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    <div className="flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0]">
                      <div className="w-8 h-8 rounded-md bg-[#157327]/10 flex items-center justify-center">
                        <Utensils className="w-4 h-4 text-[#157327]" />
                      </div>
                      <h3 className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {currentService.foodProcessingActivities.map((item, index) => (
                        <div
                          key={`${item}-${index}`}
                ...
                {Array.isArray(currentService.businessTypes) && currentService.businessTypes.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {currentService.businessTypes.map((item, index) => (
                        <div
                          key={`${item}-${index}`}
                          className="flex items-start gap-2 rounded-md bg-[#F8FAFC] border border-[#E2E8F0] p-3"
                        >
                ...
                >
                  <div className="flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0]">
                    ...
                    <h3 className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C]">
                      Why Businesses Choose PMFME Support
                    </h3>
                  </div>
