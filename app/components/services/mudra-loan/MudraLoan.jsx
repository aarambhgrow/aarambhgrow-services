...
export default function MudraLoan() {
  const services = Array.isArray(mudraLoanData?.services) ? mudraLoanData.services : [];
  const categories = Array.isArray(mudraLoanData?.categories) ? mudraLoanData.categories : [];
  const standaloneServiceIds = ["mudra-loan"];
  const standaloneServiceNames = ["Mudra Loan", "MUDRA Loan", "Pradhan Mantri Mudra Yojana", "PMMY"];
  const allowedCategoryTitles = ["Business Loans", "Business Finance", "MSME Finance", "Enterprise Finance"];
  ...
  if (!mudraLoanData || services.length === 0 || !currentService)
    return (
      <section className="w-full bg-[#F8FAFC] py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-md border border-red-200 bg-red-50 p-6">
            <h2 className="text-lg font-bold text-red-700">Mudra Loan data not found</h2>
            <p className="mt-2 text-sm text-red-600">
              Please check your <strong>data/mudra-loan.js</strong> file and make sure the services array contains your Mudra Loan service.
            </p>
          </div>
        </div>
      </section>
  ...
  const handleServiceClick = (serviceIndex, categoryTitle = "") => {
    ...
    if (typeof window !== "undefined" && window.innerWidth < 1024)
      setTimeout(() => {
        document.getElementById("mudra-loan-content")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 100);
  };
  ...
  return (
    <section id="mudra-loan" className="relative w-full bg-[#F8FAFC] text-[#03254C] font-sans py-10 sm:py-12 lg:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
        ...
        >
          ...
          >
            {mudraLoanData.hero?.title || "Mudra Loan for Small Businesses & Entrepreneurs"}
          </motion.h1>
          ...
          >
            {mudraLoanData.hero?.description}
          </motion.p>
        </motion.div>
        ...
        >
          {[
            [
              "Mudra Loan for Small Businesses & Entrepreneurs",
              "Mudra Loan is a business financing option associated with the Pradhan Mantri Mudra Yojana (PMMY), designed to facilitate institutional credit for eligible micro enterprises and entrepreneurs engaged in income-generating activities. AarambhGrow provides structured Mudra Loan support to help businesses assess their funding requirements, organize documentation, prepare project information, and improve application readiness.",
            ],
            [
              "Mudra Loan for Micro & Small Businesses in India",
              "Eligible business activities may include manufacturing, trading, services, retail, food-related businesses, transport services, repair and maintenance, and other qualifying micro-enterprise activities. The suitability of a Mudra Loan depends on applicable PMMY guidelines, applicant eligibility, business activity, lender requirements, and the proposed use of funds.",
            ],
            [
              "Mudra Loan Eligibility & Application Support",
              "Mudra Loan eligibility and documentation can vary according to the applicant, business activity, lender, and applicable scheme requirements. AarambhGrow helps review business information, funding requirements, financial details, project information, and supporting documents to prepare a more structured loan application.",
            ],
            [
              "Mudra Loan Project Report & Documentation",
              "A clear business plan and project report can help present the proposed business activity, project cost, funding requirement, expected revenue, operating expenses, and repayment capacity. Our support focuses on organizing relevant business and financial information before the loan application is submitted.",
            ],
          ].map(([title, text], i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8 items-start">
          <aside className="lg:col-span-1 bg-white border border-[#E2E8F0] rounded-md p-3 sm:p-4 shadow-[0_2px_10px_rgba(15,23,42,0.03)] lg:sticky lg:top-6">
            <div className="flex items-start gap-3 p-3 rounded-md bg-[#F8FAFC] border border-[#E2E8F0] mb-3">
              ...
              <div>
                <h3 className="text-[11px] sm:text-xs font-bold leading-[1.35] text-[#03254C]">Business Loan Solutions</h3>
              </div>
            </div>
          ...
          <div id="mudra-loan-content" className="lg:col-span-3 scroll-mt-24">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {Array.isArray(currentService.loanCategories) && currentService.loanCategories.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                      {currentService.loanCategories.map((item, index) => (
                        <div
                          key={`${item}-${index}`}
                ...
                {Array.isArray(currentService.businessActivities) && currentService.businessActivities.length > 0 && (
                  <motion.div
                    variants={cardVariants}
                    initial="hidden"
                    animate="visible"
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentService.businessActivities.map((item, index) => (
                        <span
                          key={`${item}-${index}`}
                          className="px-2.5 py-1.5 rounded-md bg-[#157327]/5 border border-[#157327]/15 text-[10px] sm:text-[11px] font-semibold text-[#157327]"
                        >
                ...
                >
                  <div className="flex items-center gap-3 pb-3 mb-4 border-b border-[#E2E8F0]">
                    ...
                    <h3 className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.1em] text-[#03254C]">
                      Why Businesses Choose Mudra Loan Support
                    </h3>
                  </div>
